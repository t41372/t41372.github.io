...
- **Page transitions use Astro ClientRouter** and native shared surface transitions. StarSky, header and footer DOM persist. Do not put view-transition names on `main` or Astro island wrappers: named ancestors create a Backdrop Root and break glass sampling. The actual glass nav and selected blog card/article are named; title/copy are separate snapshots. Preserve additive root scene crossfade and pre-capture scroll/sky updates. See `docs/navigation.md` and `THIRD_PARTY_NOTICES.md`. Do not reintroduce Swup, GSAP or glass proxies.
...
- **Preserve the original HeroSection layout and interactions.** The long authored typewriter, glitch, font cycle, draggable magnetic intro and smoke pointer interaction are intentional site features. The user explicitly rejected their removal/redesign. Implementation replacement does not authorize changing the design. Preserve `data-aurora-avoid`, the separate drag layer, and svh/lvh safeguards; retain readable SSR and avoid replay on return.
