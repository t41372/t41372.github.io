...
export default function ProjectCard({ title, description, descriptionZh, repo, media, index }: Project & { index: number }) {
    ...
    <CenterMorphModal open={open} onOpenChange={setOpen}>
      <Card className="h-full gap-8 rounded-3xl border-white/10 bg-surface/60 py-8 shadow-none glass-surface transition-colors hover:border-white/20">
        ...
        <CardContent className="flex flex-1 flex-col gap-5 px-8">
          ...
          <p className="text-pretty text-base leading-relaxed text-muted"><Localized en={description} zh={descriptionZh} /></p>
        </CardContent>
        <CardFooter className="justify-between gap-4 px-8">
          <a href={`https://github.com/${repo}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-foreground transition-colors hover:text-aurora">
            <Localized en="View repository" zh="查看源代码" /> <ArrowUpRight size={16} aria-hidden />
          </a>
          {ready && <CenterMorphModalTrigger><button type="button" className="inline-flex items-center gap-2 rounded-full border border-white/10 px-3 py-2 text-sm text-muted transition-colors hover:bg-white/5 hover:text-foreground" aria-label={language === 'zh' ? `预览 ${title}` : `Preview ${title}`}><Maximize2 size={14} aria-hidden /><Localized en="Preview" zh="预览" /></button></CenterMorphModalTrigger>}
        </CardFooter>
      </Card>
      <CenterMorphModalContent ariaLabel={title} closeButtonLabel={language === 'zh' ? `关闭 ${title}` : `Close ${title}`} className="max-w-2xl border-white/15 bg-surface p-8 sm:p-12" backdropClassName="bg-background/60">
        <p className="mb-4 font-mono text-xs uppercase tracking-widest text-muted"><Localized en="Selected project" zh="项目介绍" /> · 0{index + 1}</p>
        ...
        <p className="mb-8 text-lg leading-relaxed text-muted"><Localized en={description} zh={descriptionZh} /></p>
