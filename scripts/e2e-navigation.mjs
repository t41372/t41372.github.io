...
export async function testNavigation(browser, BASE, check) {
  for (const mobile of [false, true]) {
    ...
    check(`${label}: reduced motion leaves visible destination`, await page.locator('main h1').textContent() === 'Writings')
    await go(() => page.locator('a[data-nav-key="home"]').click())
    ...
    check(`${label}: slow response retains readable source`, await page.locator('main h1').textContent() === 'Writings')
    ...
    check(`${label}: latest navigation wins`, await page.locator('main h1').textContent() === 'Things I work on')
